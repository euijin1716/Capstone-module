...
details: {
  ...
  // (목적: 전달된 사실, 데이터, 공지를 시간 순서대로 객관적으로 요약. 단순 나열에 그치지 말고 맥락을 포함하세요.)
  
  "key_takeaways": [ "핵심 1", "핵심 2" ], 
  // (목적: 반드시 기억해야 할 결론/인사이트. 2~4개 권장. presentation_summary는 '무엇을 말했는지', key_takeaways는 '왜 중요한지'에 초점)
...
# 작업 목표
제공된 텍스트는 '의사결정' 성격의 대화입니다. 여러 대안 중 하나를 선택하거나 합의에 이르는 과정을 정리해주세요.
(주의: 단순한 문제 해결 과정과 구분하세요. 여기서는 '선택'과 '합의'가 핵심입니다.)

# 출력 JSON 포맷 및 작성 지침 (반드시 한국어로 작성)
details: {
  "decision_background": "배경 요약 문장", 
  // (목적: 왜 이 의사결정이 필요했는지 안건 배경을 한 문장으로 명시)
  
  # 개선안
  "discussed_alternatives": [ 
    "대안 1: [간단한 설명] (주요 장점: ..., 주요 단점: ...)",
    "대안 2: [간단한 설명] (주요 장점: ..., 주요 단점: ...)",
  ], 
...
1. **summary (전체 요약)**: 
   - 회의의 핵심 흐름과 결론을 5~10문장으로 요약하세요.
   - (지침: 토픽이 3개 이하면 간결하게, 5개 이상이면 충분히 상세하게 작성하세요.)

2. **decisions (결정 사항 통합)**: 
   - 각 토픽의 `segment_decisions`를 검토하고, **회의 전체에 영향을 미치는 중요한 결정**만 선별하세요.
   - **중복 제거:** 여러 토픽에서 유사한 결정 사항이 나온 경우, 하나로 통합하여 명확하게 기술하세요.
   - **선별 기준:**
     * 회의의 목적과 직접 관련된 결정
     * 향후 업무나 프로젝트에 실질적 영향을 주는 결정
     * 여러 참가자가 합의한 공식적인 결정
   - **제외 대상:** 사소한 절차적 합의(예: 점심 메뉴, 휴식 시간), 일시적 결정, 개인적 의견
   - **목표 개수:** 최대 5~8개 이내로 제한하세요.
   - **중요:** 각 결정 사항이 어느 토픽에서 나왔는지 **`related_sub_topic_id`**에 해당 토픽의 ID를 반드시 명시하세요.

3. **action_items (할 일 통합)**: 
   - 각 토픽의 `segment_action_items`를 검토하고, **구체적이고 실행 가능한 작업**만 선별하세요.
   - **중복 제거:** 동일한 작업이 여러 번 언급된 경우 하나로 합치세요.
   - **선별 기준:**
     * 명확한 작업 내용과 담당자가 있는 항목
     * 회의 목적 달성에 필수적인 작업
     * 추적 가능하고 측정 가능한 작업
   - **제외 대상:** 모호한 제안, "검토해보기" 같은 불명확한 작업, 중복된 작업
   - **목표 개수:** 최대 8~12개 이내로 제한하세요.
   - 담당자, 기한 정보를 유지하며, 역시 **`related_sub_topic_id`**를 반드시 명시하세요.


# 필수 출력 형식 (JSON Only)
반드시 아래 포맷으로 응답하세요. Markdown 코드 블록(```json)을 사용하지 말고, 순수 JSON 텍스트만 출력하세요.
모든 값은 반드시 **한국어**로 작성하세요.
...
# 중요 원칙
- **목표 개수**: 전체 회의를 **3~6개의 세부 주제**로 구조화하세요.
  * 30분 미만 회의: 2~4개
  * 30분~1시간 회의: 3~5개
  * 1시간 이상 회의: 4~6개
- **토픽 구분 기준**: 시간 흐름뿐만 아니라, **논의의 주제가 명확히 바뀌거나, 발언 점유율이 높은 주요 화자가 변경되는 지점**을 참고하여 구간을 나누세요.

# 제약 사항 (매우 중요)
1. **ID 정확성:** `start_id`와 `end_id`는 반드시 제공된 [Conversation] 텍스트 내에 존재하는 **실제 ID 숫자**여야 합니다. 임의로 생성하거나 근사치를 사용하지 마세요.
...
{{
  ...
  // (지침: 가장 최근 10~30개 발화에서 다루고 있는 주제를 파악하세요.
  // (지침: 단순히 주제명만 적지 말고, '현재 A안과 B안을 두고 격렬하게 토론 중입니다'와 같이 상황을 묘사하세요.)
  // (지침: 아직 본격적인 논의가 시작되지 않았다면 "회의 준비 중" 또는 "인사 및 아이스브레이킹"처럼 현재 상황을 설명해주세요.)

  "summary_so_far": [ "요약 1", "요약 2", "요약 3" ],
  // (지침: 회의 시작부터 현재까지의 흐름을 3~5개 문장으로 요약. 시간 순서대로 중요한 전환점 위주. 
  // (지침: 만약 논의중인 주제가 없다면 모든 내용을 "해당 없음"으로 채워주세요.)

  "key_decisions": [ "결정 사항 1" ],
  // (지침: 지금까지 명확하게 확정된 결정 사항이 있다면 기록. 없으면 빈 리스트 [])

  "catch_up_tip": "참가자가 주의해야 할 점이나 바로 확인해야 할 문서 등 조언"
  // (지침: "편하게 들으세요" 같은 형식적인 말보다는, "지금 김철수님이 예산 문제에 대해 이야기하고 있으니 그 부분에 집중하세요"처럼 실질적인 팁을 주세요.)
}}
